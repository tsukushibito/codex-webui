# Codex WebUI 公開 API 仕様書 v0.5（修正版 / 実装整合強化版）
...
- approval 実行前に、クライアントは少なくとも `approval_category`、`title`、`description`、`operation_summary` または同等情報を確認可能でなければならない
...
- 既定順序は `updated_at desc, workspace_id desc` とする
- cursor paging では上記の安定順序を前提とする
...
- 既定順序は `created_at asc, message_id asc` とする
- cursor paging では上記の安定順序を前提とする
...
- 既定順序は `occurred_at asc, sequence asc, event_id asc` とする
...
- `client_message_id` は、クライアントが同一送信を再試行するための冪等性キーである
- 同じ `session_id` と `client_message_id` の組み合わせで再送された場合、runtime / BFF は既存結果を返してよい
...
approval 一覧を返す。既定では pending のみを返す。
...
- 既定順序は `requested_at desc, approval_id desc` とする
