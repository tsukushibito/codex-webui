# Codex WebUI 内部 API 仕様書 v0.4（修正版 / 実装整合強化版）
...
- 公開 API v0.4（修正版 / app-server 整合版）を成立させるための最小で堅い内部契約を定める
...
- `failed`: App-owned 失敗終端状態
- `stopped`: App-owned 停止終端状態
...
再送時:
- 同じ `session_id` と `client_message_id` の組み合わせで再送された場合、既存の user message 結果を返してよい
...
| `operation_summary` | なし | public では通常非表示 |
