...
補足:
- 同一 workspace に別 active session が存在する場合、この response でも `can_start` は `false` になりうる
...
- stop により `canceled_approval` が `null` の場合は、approval 側の局所更新は不要としてよい
