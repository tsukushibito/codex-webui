...
- `session_id` / `message_id` / `approval_id` / `event_id` can be assumed to be opaque IDs
- In MVP, `session_id` may intentionally reuse native thread identity, but clients still treat it as an opaque string.
- `message_id`, `approval_id`, and session-scoped `event_id` are contract IDs on the app/runtime side even when native correlation information exists.
...
- `event_id`: Event identifier managed by the app/runtime contract
- `session_id`: Affiliation session
- `event_type`: Domain event type
- `sequence`: Monotonically increasing canonical number for each session
...
- `sequence` is the primary ordering contract for session-scoped events
...
On the other hand, when listing session event resources by `occurred_at`, `occurred_at` is monotonically non-decreasing in the same order as `sequence` within the same session. `occurred_at` is supplementary metadata and must not replace `sequence` as the ordering contract.
...
- `sequence` is the canonical order contract for session-scoped events
