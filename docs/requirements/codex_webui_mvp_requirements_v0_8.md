...
* Approval history can be restored 
...
* `running`: Codex is processing or progressing through the current turn.
* `waiting_input`: The assistant's previous turn has completed and continues to accept **the next regular user input.**
* `waiting_approval`: An approval request has occurred and the process has been approved/denied/stopped. 
* `completed`: A state in which the runtime has **confirmed the session as the terminal on the WebUI.
* `failed`: A state in which the runtime has determined that the session has failed to continue.
...
* `session start` is an operation that starts execution of the `created` session and transitions the state to `running`.
...
When using `occurred_at` in the session event list, `occurred_at` is monotonically non-decreasing in the same order as `sequence` within the same session.
...
* As a general rule, factual information such as message history and approval history may be given priority to the history on the app-server.
